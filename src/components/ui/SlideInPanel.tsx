...
const SlideInPanel: React.FC<SlideInPanelProps> = ({ id, title, isOpen, onClose, children, isMobile }) => {
	...
	if (isMobile) {
		...
		return (
			<div
				className={`fixed inset-0 z-50 flex flex-col bg-white dark:bg-darkgray transform transition-transform duration-300 ${isOpen ? 'translate-y-0' : '-translate-y-full'
					}`}
			>
	...
	return (
		<div
			id={id}
			className={`fixed top-0 right-0 h-full w-1/2 bg-white shadow-lg dark:bg-darkgray transform transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'
				}`}
		>
